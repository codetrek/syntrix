...
2. **Router (The Brain)**
   ...
   - **Logic**: Based on `OpKind` (Read/Write/Watch/Migrate) or potentially data attributes (Sharding Key).
   - **Examples**: `SingleRouter` (Pass-through), `SplitRouter` (Read->Replica, Write/Watch->Primary).
...
graph TD
    ...
    Router -->|Write and Watch Ops| S1[Store A]
