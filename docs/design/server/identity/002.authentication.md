...
- Populate for every authorized request: `request.auth.userId`, `request.auth.username`, `request.auth.roles` (optional), `request.time`, `request.resource.data` for writes. **Implementation note:** middlewares inject context keys (`userID`/`username`/`roles`); request mapping should use these typed keys.
