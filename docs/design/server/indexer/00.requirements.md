...
- Allow pluggable index backends (in-memory → RocksDB/external KV) without touching Query Engine logic.
...
- Expose health and [instance statistics](03.grpc.md#statistics) with equal local and remote field meanings; failed reads must remain distinct from valid zero activity. Hit/miss, lag, and rebuild telemetry require separate measurements.
