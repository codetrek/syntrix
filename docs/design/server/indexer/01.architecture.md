...
type SearchResult struct {
    Docs        []DocRef
    IndexUsed   bool    // false = no matching template, Query Engine should fallback
    IndexStatus string  // "healthy", "rebuilding", "failed"
}
...
Indexer.Search(database, plan)
    ...
    │
    ├─> Lookup index by (database, collectionPattern, templateIdentity)
    │
...
Rebuild.Start(collection, template)
    ...
    │
    ├─> Mark index as "rebuilding" (queries fall back to storage)
    │
    ...
    │       │
    │       └─> buffer.ScanFrom(startKey) → apply to index
    │
...
Indexer.Start()
    ...
    │
    ├─> Process events, update indexes
    │
    └─> If gap detected (progress too old for Puller buffer):
            │
            └─> Trigger rebuild for affected indexes
```
...
When an index is in "rebuilding" state:
- Indexer returns `ErrIndexRebuilding` with index status to Query Engine
...
For detailed index structure, OrderKey encoding, and search execution, see [Index Implementation Details](02.index.md#in-memory-index-shape-default-backend).
