...
Status: Draft
Scope: Index structure, selection, and lifecycle for the Indexer service.

For the service architecture, see [Indexer Architecture](01.architecture.md).
...
```
- Storage: load all YAML files from a configurable template directory. Changes require configuration update/restart; live mutation is outside this draft. A metadata collection remains a future storage option.
...
```text
Template files --> Group by database --> Merge templates --> Validate identities
                       |                                         |
                   orch / analytics                      Reject duplicates
```
- YAML shape (example):
```yaml
database: orch
...
Logical deletion retains a tombstone: reference/metadata, `deleted=true`, and
`data={}`. Physical cleanup removes that record later. See
...
| Physical delete | No new business deletion |

- Ordinary query indexes may exclude tombstones; replication and `ShowDeleted=true` require a template that retains them.
- Rebuild and replay use the same per-template tombstone policy.
- Retention preserves a reference, not erased sort fields or replication ordering.
- Physical cleanup does not guarantee index invalidation. See the [capability limits](../../../../.agents/notes/implemented/architecture/2026-09-08-store-watch-contract.md).
...
## Dependencies
- Data change stream via Puller gRPC subscription; index storage backend; clock/metrics; local PebbleDB event buffer.
