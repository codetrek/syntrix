...
## Responsibilities
- Apply normalized storage events idempotently, using each template's tombstone policy to choose index upsert or removal.
...
- `type Event` aligns with Puller's normalized storage event schema. Index `Delete` removes an index entry; it does not denote a raw MongoDB `delete` operation.
...
- Validation: dedupe fields, check order values, forbid duplicate (pattern+fields) definitions; optional name for human reference. `includeDeleted` is a per-template boolean; its [deletion policy](#deletion-policy) determines whether tombstone references remain indexed.
...
Syntrix logical deletion updates the stored document into a tombstone with `deleted=true` and `data={}`, retaining its reference and metadata. MongoDB physical deletion, including TTL cleanup, later removes the stored record. See [Document Deletion and Physical Cleanup](../core/storage/03.stores.md#document-deletion-and-physical-cleanup).

The current Indexer applies the full document from Puller's normalized storage event directly. `applyEventToTemplate` checks `doc.Deleted` and `tmpl.IncludeDeleted`:

| Full document and template | Index action |
| --- | --- |
| Live document | Upsert its reference and encoded order key |
| Tombstone, `includeDeleted=false` | Remove its existing index entry |
| Tombstone, `includeDeleted=true` | Upsert and retain its reference using the supplied tombstone |
| Physical delete without `fullDoc` | Skip the event; it does not signal a new business deletion |

Ordinary query indexes may exclude tombstones. An index serving replication or a query with `ShowDeleted=true` must retain them; the current query-to-template selection rejects `ShowDeleted=true` when `IncludeDeleted` is false. This requirement does not force every template to include deleted documents.

A retained tombstone has no former business-field values in `data`. Setting `includeDeleted=true` preserves an index reference; it does not recover erased values or establish replication ordering by itself. Physical cleanup does not provide another business delete event or an index invalidation guarantee. Rebuilds and replay must apply the same template policy to the available full documents.
...
- Apply: on a change event with a full document, match its collection path to templates and apply the [deletion policy](#deletion-policy); on rebuild, scan collections matching the pattern.
...
- Duplicate handling: on upsert, if `ByID` has a prior key, delete the old entry from the tree before inserting the new one. A tombstone removes the entry only when `includeDeleted=false`; otherwise it follows the upsert path. The template flag controls retention, not an implicit OrderKey field.
...
5. **Tombstone Visibility**: `ShowDeleted=true` requires a template with `includeDeleted=true`.
...
function canServeQuery(query, template):
  if query.showDeleted and !template.includeDeleted:
    return false
