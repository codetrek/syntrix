...
- Expose health and [instance statistics](03.grpc.md#statistics); hit/miss, lag, and rebuild telemetry are separate planned measurements.
...
```
- Storage: templates loaded from a directory `config/index_templates/` (default path; overridable via config/env). The system scans all `.yml` files in this directory. No live mutation in v1; changes require config update/restart. Keep interface hooks to allow future backing by metadata collection.
- **Per-Database Configuration**: Each file defines templates for a specific database using the `database` field. The same database can be defined across multiple files (useful for splitting large configurations), and templates are merged at load time.
- **Conflict Detection**: If two files define templates with the same `(database, name)` tuple, the system rejects the configuration and fails to start.
...
```
- **File naming convention**: Recommended to name files after their database for maintainability, but not enforced. The `database` field is authoritative.
...
- Multiple indexes per collection allowed if patterns are unambiguous; for the same `collectionPattern`, allow multiple templates distinguished by field list/name. Indexes must key by `(normalizedPattern, templateIdentity)` (e.g., name or fields signature), not pattern alone.
...
- Encoding: key uses fields in order; numeric big-endian sortable, strings use escaped null terminator; may append `deleted`/`version` for tie-break; cursor/startAfter share the same encoding with a version byte (OrderKey encoding versioning). A version bump means the same logical index is rebuilt with a new encoding; it is **not** a new template/index definition.
 - Encoding (OrderKey):
   - Prefix 1-byte encoding version.
   - For each indexed field (in order):
     - **Type tag (1 byte)**: `0x00`=null, `0x01`=bool, `0x02`=number, `0x03`=string. This ensures stable cross-type ordering.
     - string: escaped null terminator encoding (escape `0x00` as `0x00 0x01`, terminate with `0x00 0x00`); asc as-is; **desc invert ALL bytes (including type tag)**.
     - int/float: big-endian sortable representation; desc invert bytes (including type tag).
     - bool: 0x00 (false), 0x01 (true); desc invert (including type tag).
     - null: type tag only (0x00); desc invert.
   - Tie-breaker: append doc `ID` (2-byte length + bytes) to ensure deterministic order for equals; optional `deleted`/`version` bits if required for tombstone ordering.
...
```
[ver:1][field1...][field2...][...][id_len:2B][id bytes][optional deleted/version bits]

Field encoding (each field):
[type_tag:1B][value bytes...]

Type tags (ascending order): null=0x00 < bool=0x01 < number=0x02 < string=0x03

string field (asc): [0x03][escaped content][0x00 0x00]
  - Escape 0x00 bytes in content as 0x00 0x01
  - Terminate with 0x00 0x00
string field (desc): invert ALL bytes (including type tag and terminator)
number field: [0x02][big-endian sortable bytes]; desc invert all
bool field: [0x01][0x00 or 0x01]; desc invert all
null field: [0x00]; desc invert
```

**Why escaped null terminator for strings?** Using length-prefixed encoding (`[len:2B][content]`) causes incorrect lexicographic order: "ab" (len=2) would sort after "b" (len=1) because `0x02 > 0x01`. Escaped null terminator ensures correct byte-by-byte comparison while supporting arbitrary binary content.

**Why type tags?** Ensures stable ordering when a field has mixed types across documents. Without type tags, comparing a number to a string would produce undefined results.
...
**Two-Layer Structure**:
- Top-level: `map[string]*Database` keyed by database name for isolation and lifecycle management.
- Per-database: `map[string]*Index` keyed by `(normalizedPattern|templateIdentity)` to allow multiple indexes per collectionPattern.

**Database fields**:
  - `Name string` (e.g., `myapp`).
  - `Mu` (RW lock) guarding the database.
  - `indexes map[string]*Index` - indexes within this database.
...
- Duplicate handling: on upsert, if `ByID` has prior key, delete old entry from tree then insert new; retain/upsert logical tombstones when `includeDeleted=true`. Raw Mongo physical removal is not another business deletion.
- Concurrency: per-database and per-index RW locks; applies batch events under write lock; readers take read lock (or snapshot view if backend supports).
...
+------------------+                          +------------------+
        |                                               |
  (pattern|templateIdentity)                    (pattern|templateIdentity)
        |                                               |
+--------------------------------------+    +--------------------------------------+
| Index: users/*/chats|name:asc,age:desc|  | Index: users/*/chats|name:asc,age:desc|
+--------------------------------------+    +--------------------------------------+
...
+--------------------------------------+    +--------------------------------------+
         ↑ Same pattern, different databases = completely isolated indexes

OrderKey layout:
[ver:1B][field1...][field2...][...][id_len:2B][id bytes][optional deleted/version bits]
- each field: [type_tag:1B][value bytes...] (type tags: null=0x00, bool=0x01, number=0x02, string=0x03)
- strings: [0x03][escaped content][0x00 0x00]; desc invert ALL bytes
- nums: [0x02][big-endian sortable]; desc invert all
- bool: [0x01][0x00/0x01]; desc invert all
...
This section defines how Indexer selects the best template based on query's `orderBy` and `filters`. Query Engine passes a `Plan` to Indexer; Indexer internally applies these rules to select the optimal template.
...
4. **Range Filter Last**: At most one range field, positioned after all equality fields; intersect its range predicates before scanning. It "consumes" the index.
...
For each field usable by the selected index, all `==`, `>`, `>=`, `<`, and `<=`
predicates form one conjunction. Selecting only one equality or overwriting a
bound can widen that conjunction, so bounds must retain its intersection.

| Predicates | Intersection rule |
|---|---|
| Repeated equality | All encoded values must agree |
| Equality and ranges | The equality value must satisfy every lower and upper constraint |
| Multiple lower bounds | Keep the greatest value; a strict bound wins an equal-value tie |
| Multiple upper bounds | Keep the least value; a strict bound wins an equal-value tie |
| Lower value exceeds upper value | Empty result |
| Lower equals upper | Include that value only when both bounds are inclusive |

Predicate order does not affect the intersection. Comparison uses the existing
value encoding. Descending fields translate the same logical intersection into
reversed key bounds; strictness and equality-prefix handling must include or
exclude every key with the boundary value, including document-ID suffixes.
Index selection and cursor decoding precede bound construction. Contradictions
on usable fields return an empty result without a backend index search; backend
readiness errors are therefore not observed for that empty result. Encoding
errors on visited usable fields still propagate.

The [same-field intersection decision](../../../../.agents/notes/implemented/bug-fix/2026-09-11-indexed-filter-intersection.md)
owns the rationale and limits. This behavior does not add cross-field residual
evaluation, change number encoding, or expand the supported operator set.
...
4. **Tie-breaker**: If tied after above criteria, select the template with lexicographically smallest name (deterministic, no error).
...
function canServeQuery(query, template):
  ...
  // Step 3: Check range field (at most one, on next field)
  rangeField = nil
  remainingFilters = query.filters.exclude(eqFields)
  for f in remainingFilters:
    if f.op in [gt, lt, gte, lte]:
      if rangeField != nil and rangeField != f.field: return false
      rangeField = f.field
...
function selectBestIndex(query, templates):
  ...
  if len(ties) > 1:
    // Tie-breaker: select lexicographically smallest template name
    ties.sortBy(t => t.name)
    return ties[0].template
...
function search(database, plan):
  // Step 1: Select best template via Query-to-Index matching
  template = selectBestTemplate(plan)  // uses plan.Collection, OrderBy, Filters
  if template == nil: return ErrNoMatchingIndex

  // Step 2: Derive index key from template
  pattern = normalize_vars(template.collectionPattern)  // e.g., users/*/chats
  templateIdentity = template.name or template.fieldsSignature

  cursor = nil
  if plan.startAfter != "":
    cursor = decodeOrderKey(plan.startAfter)
  (lower, upper, empty) = buildBounds(plan) // intersect same-field constraints
  if empty: return []
  cursorStart = lower
  if cursor != nil: cursorStart = cursor

  // Step 3: Lookup index for nonempty bounds
  idx = lookupIndex(database, pattern, templateIdentity)
  if idx == nil: return ErrIndexNotReady

  it = idx.Tree.iterFrom(cursorStart)
  results = []
  ...
  return results
```
Notes: `buildBounds` encodes the intersected prefix/range per usable index field;
Query batches document fetches by returned IDs. Complete residual evaluation
remains in the [indexed filter proposal](../../../../.agents/notes/proposed/bug-fix/2026-09-07-indexed-query-filter-semantics.md).
Version check for cursor/orderkey happens before `search` (if mismatch → IndexNotReady).
...
- Stats: sample loaded template count, matching-event processing count, and local Unix seconds of the latest count update. Local and remote callers share the [statistics contract](03.grpc.md#statistics); these values do not establish successful indexing, checkpoint progress, or lag. Hit/miss, lag, and rebuild-duration telemetry remain planned.

#### Rebuild/Apply Ordering
- During rebuild: record buffer position (startKey), scan storage with pagination, then replay buffered events from startKey before marking index healthy.
...
- Each consumer maintains its own checkpoint (per collection) persisted externally; puller only guarantees ordered delivery per collection.
...
- Backpressure: if a consumer lags beyond thresholds, pause its deliveries; mark index as "needs rebuild" rather than endlessly buffering. Resume from checkpoint after rebuild.
...
| Multi-database isolation | Index key includes database: `(database, normalizedPattern, templateIdentity)`. Same collection pattern in different databases results in separate indexes. |
