...
internal/indexer/
  ...
  persist_store/          # Persistent storage implementation
    pebble.go             # PebbleStore (PebbleDB)
...
Defined in `store` package:

```go
// internal/indexer/store/store.go
...
- An in-memory operation shadows the persisted entry for the same document; pending operations override flushing operations.
- A matching update appears once at its sampled order-key position. A deletion or filtered-out update suppresses the persisted entry without a replacement.
- Deduplication tracks returned documents; skipping a shadowed disk entry must not suppress its qualifying replacement. See the [search merge decision](../../../../.agents/notes/implemented/bug-fix/2026-09-09-indexer-pending-search-results.md).
