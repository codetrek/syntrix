...
| Tenant ID | `tenant` | string | Yes | Derived from `fullDoc.tenantId` |
...
| Full Document | `fullDoc` | object | No | `storage.Document` shape. Required for insert/update/replace |
...
The `fullDoc` field MUST adhere to the `storage.Document` JSON schema.
