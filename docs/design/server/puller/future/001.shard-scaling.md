...
Consumer subscribes with:
  - shard_by: ["database", "collection"]  # Fields to hash
  - shard_count: 4                       # Total shards
  - shard_ids: [0, 1]                    # Which shards this consumer wants

Puller receives event:
  - Extracts field values: database="acme", collection="orders"
...
| Shard Key | Fixed (database + collection) | Consumer-defined fields |
...
message ShardConfig {
  repeated string shard_by = 1;    // Ordered list of fields to hash: ["database", "collection"]
...
// Example: ["database", "collection"] produces different hash than ["collection", "database"]
...
streamer:
  puller:
    ...
    shard:
      shard_by: ["database"]  # Shard by database only
      shard_count: 4
      shard_ids: [0]
```
...
```yaml
# Shard by database only (all collections for a database go to same shard)
shard:
  shard_by: ["database"]
  shard_count: 4
  shard_ids: [0]

# Shard by database + collection (finer granularity)
shard:
  shard_by: ["database", "collection"]
  shard_count: 8
  shard_ids: [0, 1]
...
```text
VIP databases get dedicated instances:

  Engine-VIP: shard_by=["database"], filter database in ["acme", "bigcorp"]
  Engine-Standard: shard_by=["database"], filter database not in ["acme", "bigcorp"]
```
...
- [Puller Architecture](../01.architecture.md) - Current Puller design (without sharding)
- [Puller Design Details](../02.adaptive-consumption.md) - Implementation details
