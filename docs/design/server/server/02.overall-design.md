...
## 5. Implemented Service Manager Integration

The preceding network sketches are design material. Production service assembly
registers handlers and transport adapters during `Manager.Init(ctx)`; the Puller
adapter uses the shared subscription service and does not open its own listener.

`Manager.Start(bgCtx) error` starts local Puller ingestion synchronously first.
Each backend durably establishes its saved-token or initial-operation-time
boundary before its watch reports ready. A startup failure or cancellation
returns before Streamer and Indexer begin consuming. The manager then launches
the unified network server, starts Streamer and Indexer, and launches the
remaining background workers. Network serving and later worker failures retain
their asynchronous logging behavior; manager startup does not claim a separate
synchronous listener-bind acknowledgement.

The caller must cancel `bgCtx` and call `Manager.Shutdown` after a synchronous
startup error. The CLI uses this ordering:

```text
Manager.Start(bgCtx)
        |
        +-> success: wait for shutdown signal
        |
        +-> error: cancel bgCtx
                       |
                       v
...
Cleanup uses `Server.ShutdownTimeout` with a fresh context so cancellation of
running workers does not remove their cleanup allowance. Shutdown closes owned
subscription streams before gRPC graceful shutdown waits on those streams.
Storage sync may outlive a caller deadline while retaining ownership of its
resources; cleanup cannot imply the write was rolled back.

The [lifecycle architecture](01.architecture.md) owns the complete readiness and
shutdown contract. Production sources are
[Manager.Start](../../../../internal/services/manager_start.go),
[Manager.Shutdown](../../../../internal/services/manager_shutdown.go), and
[the CLI](../../../../cmd/syntrix/main.go).
