...
### 3.2 Event Ingestion
- **Source**: Consume normalized storage events from the **Puller** service. Their `insert`, `update`, `replace`, and `delete` operation types describe MongoDB operations.
- **Logical deletion**: Syntrix deletion creates a tombstone through a MongoDB update: `deleted=true`, `data={}`, with the document reference and metadata retained. A tombstoned `update` or `replace` must become a business `EventDelete` carrying that tombstone.
- **Physical cleanup**: A raw MongoDB `delete`, including later TTL cleanup, must be ignored by business-event transformation. It must not cause another delete notification. The current Puller pipeline and normalizer retain this raw operation; `events.Transform` returns `ErrDeleteOPIgnored` at the consumer boundary.
- **Recreation**: Creating a document at the path of a retained tombstone uses the storage `Create` operation, whose current implementation performs `ReplaceOne`. A `replace` carrying a live document becomes `EventCreate`; a live `update` remains `EventUpdate`.

[Document Deletion and Physical Cleanup](../core/storage/03.stores.md#document-deletion-and-physical-cleanup) defines the shared contract. Current ingestion classifies the full document supplied by Puller; it does not reconstruct event-time transitions from `updateDesc`. `UpdateLookup` and raw-event coalescing therefore must not be treated as guarantees that every logical transition is preserved.

### 3.3 Event Matching
- **Query Matching**: Determine which subscriptions match a given change event.
- **Tombstone payload**: Logical delete matching receives retained reference/metadata and cleared business data. The tombstone does not provide the document's former business fields for predicate evaluation.
