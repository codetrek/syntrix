...
```text
MongoDB -- one stream --> Puller --> Streamer
                                       |
                          +------------+------------+
                          v                         v
                       Gateway 1                 Gateway 2
...
Streamer consumes Puller's normalized MongoDB operations and owns business conversion:

| Source event | Required business outcome |
| --- | --- |
...
- Logical deletion and physical cleanup are distinct source changes; cleanup must not produce a second business delete.
- Lookup payloads and coalesced replay do not guarantee every historical business transition. See the [capability limits](../../../../.agents/notes/implemented/architecture/2026-09-08-store-watch-contract.md).
