...
### 4.1 Performance
- **Latency**: Streamer processing (Ingest -> Match -> Send) should be < 10ms (p99). This target excludes the preceding Puller batching and synchronous commit; end-to-end change latency must measure both stages.
- **Throughput**: Capable of handling 10k+ events/sec per node.

### 4.2 Reliability
- **At-Least-Once Delivery**:
    - **Upstream (Puller -> Streamer)**: Preserve the complete processed Puller marker after successful processing. Local and gRPC ingestion handle progress-only envelopes and terminal source errors. Restart replay requires durable storage of that marker; this remains the [durable progress task](../../../../.agents/notes/proposed/architecture/2026-09-07-streamer-durable-progress.md).
    - **Downstream (Streamer -> Gateway)**: A transport send is not client receipt. Terminal upstream failure closes owned gateway streams; gateways re-register after reconnection. Complete client continuity remains the [client resume task](../../../../.agents/notes/proposed/feature/2026-09-07-realtime-client-resume.md).
