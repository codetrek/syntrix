...
*   **Protocol**: Shared Puller subscription, through direct calls or gRPC server streaming.
*   **Responsibility**:
    *   Deserialization.
    ...
    *   **Ordering**: Preserves each source generation's committed sequence from Puller.
    *   **Progress**: Handles initial/progress-only envelopes and advances in-memory progress only after successful processing.
    *   **Failure**: Terminal subscription or processing failure stops ingestion and closes owned gateway streams. `Err` and `Done` expose the cause.

[Puller subscriptions](../puller/02.adaptive-consumption.md) define replay and
terminal history errors. Durable Streamer restart progress remains in the
[progress proposal](../../../../.agents/notes/proposed/architecture/2026-09-07-streamer-durable-progress.md);
gateway re-registration alone does not prove client delivery continuity.
