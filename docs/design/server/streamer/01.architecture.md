...
### 3.1 Ingestion Layer
*   **Role**: Receives normalized MongoDB storage events from Puller and converts them to Syntrix business events before matching.
*   **Protocol**: gRPC Server Streaming.
*   **Responsibility**:
    *   Deserialization.
    *   **Logical Delete Transformation**: Calls `events.Transform`; an `update` or `replace` carrying `deleted=true` becomes `EventDelete` with the tombstone. The tombstone retains reference/metadata and has `data={}`.
    *   **Physical Cleanup Filtering**: Treats `ErrDeleteOPIgnored` for a raw `StoreOperationDelete` as no business event. Physical deletion, including TTL cleanup, can arrive through Puller's normalized stream and must not produce another logical delete.
    *   **Recreation Handling**: A `replace` carrying a live document becomes `EventCreate`. This matches storage `Create` replacing a retained tombstone through `ReplaceOne`. A live `update` becomes `EventUpdate`.
    *   **Ordering**: Preserves the received order of business events that survive transformation.

The shared [Document Deletion and Physical Cleanup](../core/storage/03.stores.md#document-deletion-and-physical-cleanup) contract owns these semantics. The current transformer classifies the supplied `fullDoc`; it does not reconstruct historical transitions from `updateDesc`. Puller's `UpdateLookup` result and any raw-event coalescing do not establish preservation of each logical transition.
...
For logical deletion, matching evaluates the tombstone's reference/metadata and cleared business data. It does not recover former business fields from the tombstone.
...
2.  **Ingest**: Puller sends a normalized raw `insert` with a full document containing `data: {chat_id: 1, text: "hi"}`.
3.  **Transform**: `events.Transform` creates an `EventCreate`. Tombstoned updates/replacements become `EventDelete`; physical deletes stop at this boundary without entering matching.
4.  **Match**: Engine sees `chat_id: 1` in the business event, looks up Index, finds Gateway A is interested.
5.  **Route**: Streamer sends the matched event to Gateway A via the established stream.
6.  **Progress**: The current Puller ingestion loop advances its in-memory progress marker after processing each received event, including ignored physical deletes.
