...
1) DocumentWatcher loads the complete opaque Puller progress marker and opens a local or remote `Subscription`; its `WatcherStream.Next` carries events, progress-only envelopes, and terminal errors.
...
6) Only successfully evaluated and published events advance completed progress. The asynchronous saver coalesces that aggregate source/generation/sequence marker. Processing or save failure stops consumption. This is the current publisher-success boundary; durable scheduling/outbox remains separate work.
7) The watcher receives the configured Puller source scope. Rule evaluation selects the logical database and collection. Initial and progress-only envelopes can advance progress without a document.
8) Checkpoint key: `sys/checkpoints/trigger_evaluator`. The marker contains independent physical-source positions. On terminal source failure the service cancels its saver before joining it. See [evaluator/01.checkpoint.md](evaluator/01.checkpoint.md) for bootstrap, shutdown, and remaining scheduling guarantees.
...
- Recovery semantics: a missing checkpoint permits current-head bootstrap only with `StartFromNow`; corrupt, unreadable, unsupported, or expired checkpoints fail explicitly. Clearing a checkpoint is an operator recovery decision and cannot reconstruct historical Webhook effects.
