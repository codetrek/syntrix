...
- Puller returns opaque source/generation/sequence progress across all sources.
- `DocumentWatcher.Watch(ctx)` returns a `WatcherStream` with `Next(ctx)` and `Close()`.
- Initial and progress-only envelopes remain meaningful; runtime errors reach the evaluator.
- Evaluation/publication failures stop checkpoint advancement; saver failure stops consumption.
- Logical database selection happens during rule evaluation; checkpoints preserve the aggregate marker.
...
├── nats.go           # NATS connection helpers
