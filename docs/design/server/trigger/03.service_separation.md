...
- Puller returns one opaque aggregate source/generation/sequence marker.
- The watcher exposes `Next`/`Close` and preserves initial/progress-only envelopes and terminal failures.
- Rule evaluation selects logical databases; checkpoint advancement follows successful processing.
- Saver errors stop ingestion, and source termination cancels the saver before joining it.
