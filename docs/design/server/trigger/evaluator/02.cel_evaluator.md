...
| `doc` | map | Current business data; cleared on logical deletion, with document identity retained in the event |
...
`delete` means Syntrix logical deletion; physical Mongo cleanup is ignored.
This example requires the proposed [before-image capability](../../../../../.agents/notes/proposed/feature/2026-09-07-trigger-before-images.md);
a tombstone's cleared business data cannot supply `before.balance`.
