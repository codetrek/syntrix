...
The evaluator captures each rule's effective `Timeout` in the task: an omitted
or zero value becomes 30 seconds, and a positive value is copied unchanged.
Negative timeouts fail rule activation before replacing the previous active
rules. The captured value remains unchanged across retries, so later rule edits
apply to newly created tasks. See the
[timeout decision](../../../../../.agents/notes/implemented/bug-fix/2026-09-07-trigger-rule-timeouts.md)
for the validation and deadline boundaries.
