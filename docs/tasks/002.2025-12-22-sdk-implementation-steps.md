...
- Ensure optional client-provided id on create; query posts to `/v1/query` with `Query` shape.
...
- Implement class in `src/clients/trigger-client.ts` with `preIssuedToken` only (no refresh). Use trigger endpoints (`/v1/trigger/...`).
- Enforce required id on create; `get` tolerant to empty docs array; implement `batch(writes)` to `/v1/trigger/write`.
