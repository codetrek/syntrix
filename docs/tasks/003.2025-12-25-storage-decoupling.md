...
- Close lifecycle: enforce shutdown order (stop services → close providers); make `Close` idempotent and close partial resources on construction failure.
...
- UserStore
  - CreateUser(ctx, user *auth.User) error
  - GetUserByUsername(ctx, username string) (*auth.User, error)
  - GetUserByID(ctx, id string) (*auth.User, error)
  - UpdateUserLoginStats(ctx, id string, lastLogin time.Time, attempts int, lockoutUntil time.Time) error
  - ListUsers(ctx, limit, offset int) ([]*auth.User, error)
  - UpdateUser(ctx, user *auth.User) error
...
- Auth constructor
  - `auth.NewAuthService(userStore UserStore, revStore TokenRevocationStore, tokenSvc ...)`
...
- Manager init (single backend, no routing yet)
  ...
  authSvc := auth.NewAuthService(authProv.Users(), authProv.Revocations(), tokenSvc)
  ```
...
- Unit test with fake stores (no Mongo)
  ...
  authSvc := auth.NewAuthService(fakeUsers, fakeRevs, tokenSvc)
