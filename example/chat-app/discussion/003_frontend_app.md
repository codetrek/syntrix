...
### 2.2 Sync Strategy
-   **Global Sync**: On startup, sync `users/demo-user/chats` to populate the sidebar.
-   **Dynamic Sync**: When a user selects a chat, dynamically create an RxCollection for `users/demo-user/chats/<chatId>/messages` and start replication. This ensures we only load relevant messages.
