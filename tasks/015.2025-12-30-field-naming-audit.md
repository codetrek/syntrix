...
| 4 | `internal/identity/types/types.go` | 28 | Claims.TenantID | `json:"tid"` | `json:"tenantId"` | **Keep tid/oid (documented exception)** |
| 5 | `internal/identity/types/types.go` | 29 | Claims.UserID | `json:"oid"` | `json:"userId"` | **Keep tid/oid (documented exception)** |
...
**JWT Token Format**: Keep `tid`/`oid` in claims to avoid breaking existing tokens and integrations.
...
- [ ] Token validation accepts both old and new formats (migration period)
- [ ] All auth tests pass
...
| Risk | Mitigation |
| ---- | ---------- |
| JWT token invalidation | Migration period with dual format support |
