...
func TestSuspendedDatabaseRejectsWrites(t *testing.T) {
	...
	// Status is database-wide, so this test owns its database while other tests
	// continue using the shared default database.
	...
	adminPath := "/admin/databases/id:" + database.ID
	t.Cleanup(func() {
